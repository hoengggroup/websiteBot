...
myWebpage = Webpage("https://tassilo-schwarz.com/test-page/",30)
...
for current_wpbg_name in webpages_dict:
    ...
    if(elapsed_time > datetime.timedelta(seconds = current_wbpg.t_sleep)):
        ...
        if(current_hash != current_wbpg.last_hash):
            logger.info("Website hash different.")
            # 3.1 determine difference using DP (O(m * n) ^^)
            old_words_list = string_to_wordlist(current_wbpg.current_text)
            ...
            current_wbpg.current_hash = current_hash
            current_wbpg.current_text = current_text
